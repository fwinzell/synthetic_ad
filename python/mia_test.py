...
from eval_benchmark import load_real_data, load_synthetic_with_imputation, generate_synthetic_with_imputation
from train_svm import data_loading, impute_missing_values
from DataSynthesizer.DataGenerator import DataGenerator


class MIARFClassifier(MetricClass):
    ...
    def evaluate(self, num_eval_iter=5) -> float | dict:
        ...
        try:
            ...
            for _ in range(num_eval_iter):
                ...
                cls = self.fit_rfc(X_train, y_train)
                # Get predictions
...
def mia_test(syn_dir, generate=False):
    ...
    miarfc = MIARFClassifier(real_df, syn_dict, test_df, cat_cols=categorical_attributes, num_cols=[], nn_dist="euclidean", analysis_target=tar, do_preprocessing=False, verbose=True)
    results = miarfc.evaluate(num_eval_iter=1)
...
def main_mia_test():
    for eps in [200]:
        ...
        syn_dir = f'/home/fi5666wi/Python/WASP-DDLS/DS-synthetic-data/degree{degree}_eps_{eps}/'
        ...
        results = mia_test(syn_dir, generate=False) #benchmark_utility_and_privacy(syn_dir, generate=False)

        print("====== Results ======")
        print("MIA precision: ", results["MIA precision"])
        print("MIA precision se: ", results["MIA precision se"])
        print("MIA recall: ", results["MIA recall"])
        print("MIA recall se: ", results["MIA recall se"])
        print("MIA macro F1: ", results["MIA macro F1"])
        print("MIA macro F1 se: ", results["MIA macro F1 se"])
